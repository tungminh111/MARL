...
class QAgent(TrainableAgent):
    ...
    def update_model(self, t):
        ...
        for i in range(len(batch.observation)):
            self.id = i
...
class MinimaxQAgent(QAgent, MATrainable):
    ...
    def update_q(self, curr_value, target_value, batch):
        ...
        self.policy.Q.q_table[batch.observation[self.id][self.index], batch.action[self.id][self.index], batch.action[self.id][1-self.index]]  = curr_value + self.lr * (target_value - curr_value)

    def update_target_model(self):
        self.target_policy = copy.deepcopy(self.policy)

    def target(self, Q, joint_batch):
        next_obs  = joint_batch.next_observation[self.id][self.index]
        next_value = Q(next_obs).max()
        rew = torch.tensor(joint_batch.reward[self.id]).squeeze(0)[self.index].float()
        not_dones = torch.tensor(1.-joint_batch.done_flag[self.id]).squeeze(0)[self.index].float()
        target_value = rew + not_dones * self.gamma * next_value
        return target_value

    def value(self, observation, action):
        return self.policy.Q.q_table[observation[self.id][self.index], action[self.id][self.index], action[self.id][1-self.index]]
